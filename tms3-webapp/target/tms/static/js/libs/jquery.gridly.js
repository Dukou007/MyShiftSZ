...
(function() {
  ...
  Draggable = (function() {
    ...
    Draggable.prototype.began = function(event) {
      ...
      if(event.target.className=="delete-dashboard-item"){
    	  setTimeout(function(){event.target.click()},20)
      }
    ...
    Draggable.prototype.moved = function(event) {
      ...
      if(!ismoved){
    	  ismoved = true;
          moveleft = this.coordinate(event).pageX;
          movetop = this.coordinate(event).pageY;
      }
      if(ismoved && !(moveleft==this.coordinate(event).pageX && movetop == this.coordinate(event).pageY)){
    	  event.preventDefault();
	      event.stopPropagation();
	      this.$target.css({
  ...
  Gridly = (function() {
    ...
    Gridly.prototype.position = function($element, columns) {
      ...
      size = Math.round(this.size($element));
      height = Infinity;
      column = 0;
