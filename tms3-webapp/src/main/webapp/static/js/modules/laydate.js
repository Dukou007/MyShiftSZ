(function(factory) {
	...
	Dates.run = function(options) {
		...
		//as.elemv = /textarea|input/.test(elem.tagName) ? 'value' : 'innerHTML';
		as.elemv = 'value';
		...
		if (even && target.tagName) {
			...
			Dates.stopMosup(even.type, elem[0]);
			Dates.stopmp(even);
			Dates.view(elem[0], options);
			Dates.reshow();
		} else {
	...
	Dates.viewDate = function(Y, M, D) {
		var S = Dates.query,
			log = {};
		var timeZoneId = $("#timeZoneId").find('option:selected').val();
    	
		var De=new Date();
    	if(typeof(timeZoneId)!='undefined' && timeZoneId!=''){
    		$.ajax({
        		url: _WEB_SITE_ +'/address/service/getNowTimeByTimeZone',
        		type: 'GET',
        		cache: false,
        		async:false,
        		data: {
        		...
        		success: function(data) {
        			...
        			De=new Date(data.slice(0,4),data.slice(5,7)-1,data.slice(8,10),data.slice(11,13),data.slice(14,16),data.slice(17,19));
        			ajaxDe=null;
        			ajaxDe=De.clone();
		...
		if (D == Dates.ymd[2] && M == Dates.ymd[1] && Y == Dates.ymd[0]) {
		...
		var _De = De;
		log.times = [
			Dates.inymd[Dates.elemIndexMap.hour] | 0 || _De.getHours(),
			Dates.inymd[Dates.elemIndexMap.minute] | 0 || _De.getMinutes(),
			Dates.inymd[Dates.elemIndexMap.second] | 0 || _De.getSeconds()
		];
	...
	Dates.view = function(elem, options) {
		...
		if (!Dates.box) {
			...
			+ '<div class="laydate_bottom">' + '<ul id="laydate_hms">' + '<li class="laydate_sj">Time</li>' + '<li><input readonly></li>' + '<li>:<input readonly></li>' + '<li>:<input readonly></li>' + '</ul>' + '<div class="laydate_time" id="laydate_time"></div>' + '<div class="laydate_btn">' + '<a id="laydate_clear">Clear</a>' + '<a id="laydate_today">Now</a>' + '<a id="laydate_ok">OK</a>' + '</div>' + (config.isv ? '<a href="#" class="laydate_v" target="_blank">laydate-v' + laydate.v + '</a>' : '') + '</div>';
		...
		$('#laydate_YY,#laydate_MM,#laydate_table,.laydate_top,#laydate_hms li').removeClass('laydate_hide');
		if (Dates.options.format.indexOf('YYYY') == -1) {
			$('#laydate_YY').addClass('laydate_hide');
		}
		if (Dates.options.format.indexOf('MM') == -1) {
			$('#laydate_MM').addClass('laydate_hide');
		}
		if (Dates.options.format.indexOf('MM') == -1 && Dates.options.format.indexOf('YYYY') == -1 && Dates.options.format.indexOf('DD') == -1) {
			$('.laydate_top').addClass('laydate_hide');
		}
		if (Dates.options.format.indexOf('DD') == -1) {
			$('#laydate_table').addClass('laydate_hide');
		}
		if (Dates.options.format.indexOf('ss') == -1) {
			$('#laydate_hms li:eq(3)').addClass('laydate_hide');
		}
		if (Dates.options.format.indexOf('hh') == -1) {
			$('#laydate_hms li:eq(1)').addClass('laydate_hide');
		}
		if (Dates.options.format.indexOf('mm') == -1) {
			$('#laydate_hms li:eq(2)').addClass('laydate_hide');
		}
	...
	//返回最终日期
	Dates.creation = function(ymd, hide, ev) {
		var S = Dates.query,
			hms = Dates.hmsin;
		...
		if (!hide) {
			Dates.close();
			typeof Dates.options.choose === 'function' && Dates.options.choose(getDates, ev);
		}
	};
	...
	Dates.events = function() {
		...
		Dates.each(S('#laydate_table td'), function(i, elem) {
			Dates.on(elem, 'click', function(ev) {
				if (!Dates.hasClass(this, as[1])) {
					...
					Dates.stopmp(ev);
					Dates.creation([this.getAttribute('y') | 0, this.getAttribute('m') | 0, this.getAttribute('d') | 0], 1, Dates.elem);
				}
			});
		});
		...
		Dates.on(as.otoday, 'click', function(ev) {
			var now = ajaxDe ? ajaxDe : new Date();

		
			Dates.elem[as.elemv] = laydate.now(0, Dates.options.format);

			typeof Dates.options.choose === 'function' && Dates.options.choose(Dates.elem[as.elemv], Dates.elem);
			Dates.close();
		...
		//确认
		as.ok = S('#laydate_ok');
		Dates.on(as.ok, 'click', function(ev) {
			if (Dates.valid) {
				Dates.creation([Dates.ymd[0], Dates.ymd[1] + 1, Dates.ymd[2]], null, Dates.elem);
			}
		});
			...
			Dates.each(span, function(i, elem) {
				if (set) {
					...
					Dates.on(elem, 'click', function(ev) {
						...
						if (Dates.valid) {
							Dates.creation([Dates.ymd[0], Dates.ymd[1] + 1, Dates.ymd[2]], 1, Dates.elem);
						}
					});
				}
