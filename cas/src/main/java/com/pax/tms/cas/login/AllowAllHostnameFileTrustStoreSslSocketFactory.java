...
@Component("allowAllHostnameTrustStoreSslSocketFactory")
@SuppressWarnings("deprecation")
public class AllowAllHostnameFileTrustStoreSslSocketFactory extends SSLConnectionSocketFactory {

	private static final Logger LOGGER = LoggerFactory.getLogger(FileTrustStoreSslSocketFactory.class);

	/**
	 * Instantiates a new trusted proxy authentication trust store ssl socket
	 * factory. Defaults to {@code TLSv1} and
	 * {@link SSLConnectionSocketFactory#BROWSER_COMPATIBLE_HOSTNAME_VERIFIER}
	 ...
	 */
	@Autowired
	public AllowAllHostnameFileTrustStoreSslSocketFactory(
			@Value("${http.client.truststore.file:classpath:truststore.jks}") final File trustStoreFile,
			@Value("${http.client.truststore.psw:changeit}") final String trustStorePassword) {
	 ...
	 */
	public AllowAllHostnameFileTrustStoreSslSocketFactory(final File trustStoreFile, final String trustStorePassword,
	...
	private static class CompositeX509KeyManager implements X509KeyManager {
		...
		/**
		 * Represents an ordered list of {@link X509KeyManager}s with
		 * most-preferred managers first.
		 * 
		 * @param keyManagers
	...
	/**
	 * Represents an ordered list of {@link X509TrustManager}s with additive
	 * trust. If any one of the composed managers trusts a certificate chain,
